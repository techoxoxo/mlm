...
function Field({ name, label, value, hint, min, max }: { name: string; label: string; value: number; hint?: string; min?: number; max?: number }) {
  return (
    <div>
      ...
      <input name={name} type="number" min={min} max={max} defaultValue={value} className="input" />
...
export default async function SettingsAdmin() {
  ...
  return (
    <form action={updateSettingsAction} className="card" style={{ padding: 26, maxWidth: 640 }}>
      <h3 style={{ margin: "0 0 4px", fontSize: 16 }}>Economy controls</h3>
      ...
      <div style={{ fontSize: 11, letterSpacing: "0.1em", textTransform: "uppercase", color: "var(--gold)", fontWeight: 700, marginBottom: 12 }}>
      ...
      <label style={{ display: "flex", alignItems: "center", gap: 10, fontSize: 14, marginTop: 18 }}>
