...
function Field({ name, label, value, type = "number" }: { name: string; label: string; value: string | number; type?: string }) {
  return (
    <div>
      ...
      <input name={name} type={type} defaultValue={value} className="input" style={{ width: "100%" }} />
    </div>
  );
}
...
export default async function SlabsAdmin() {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 18, maxWidth: 800 }}>
      <SettingsLockWrapper title="Slabs Configuration">
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
          {rows.map((s) => (
            <form key={s.level} action={updateSlabAction} className="card" style={{ padding: 22, background: "rgba(255,255,255,0.02)" }}>
