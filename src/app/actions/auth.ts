...
export async function sendUserOtpAction(): Promise<{ ok: boolean; error?: string; devMode?: boolean }> {
  try {
    const session = await getSession();
    if (!session || !session.uid) return { ok: false, error: "Not authenticated" };
    ...
    if (!emailEnabled) {
      ...
      await connection.set(`otp:${cleanEmail}`, "123456", "EX", 900);
      console.log(`[EMAIL DISPATCH - DEV] Target: ${cleanEmail} | Verification Code: 123456`);
    ...
    console.error("sendUserOtpAction failed:", err);
